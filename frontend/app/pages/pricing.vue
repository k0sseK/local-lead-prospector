...
<template>
	<div class="bg-brand-dark min-h-screen">
		<!-- Header -->
		<div class="container mx-auto px-6 lg:px-12 max-w-5xl pt-24 pb-16 text-center">
			<div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold tracking-wider uppercase bg-brand-teal/10 border border-brand-teal/30 text-brand-green mb-6">
				Cennik
			</div>
			...
			<p class="text-slate-400 text-lg max-w-xl mx-auto">
				Zacznij bezpłatnie i przejdź na Pro gdy jesteś gotowy na pełną automatyzację.
			</p>
		</div>
		...
		<div class="container mx-auto px-6 lg:px-12 max-w-5xl pb-24">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
				...
				<!-- Free -->
				<div class="relative rounded-2xl border border-white/10 bg-white/5 p-8 flex flex-col">
					<div class="mb-6">
						<p class="text-sm font-semibold uppercase tracking-wider text-slate-400 mb-2">Free</p>
						<div class="flex items-end gap-2">
							<span class="text-5xl font-extrabold text-white">0 PLN</span>
							<span class="text-slate-500 mb-1">/miesiąc</span>
						</div>
						<p class="text-slate-500 text-sm mt-2">Na zawsze darmowy</p>
					</div>

					<ul class="space-y-3 flex-1 mb-8">
						<li v-for="feature in freePlan" :key="feature" class="flex items-start gap-3 text-sm text-slate-300">
				...
				<!-- Pro -->
				<div class="relative rounded-2xl border border-brand-green/40 bg-brand-green/5 p-8 flex flex-col shadow-[0_0_40px_rgba(56,239,125,0.08)]">
					<!-- Badge -->
					...
					<ul class="space-y-3 flex-1 mb-8">
						<li v-for="feature in proPlan" :key="feature" class="flex items-start gap-3 text-sm text-slate-300">
							<Check class="w-4 h-4 text-brand-green flex-shrink-0 mt-0.5" />
							{{ feature }}
						</li>
					</ul>
			...
			<div class="mt-20 max-w-2xl mx-auto">
				...
				<div class="space-y-6">
					<div class="border-b border-white/10 pb-6">
						<h3 class="text-white font-semibold mb-2">Jak działają limity?</h3>
						<p class="text-slate-400 text-sm leading-relaxed">Limity resetują się automatycznie pierwszego dnia każdego miesiąca. Bieżące zużycie widzisz w dashboardzie.</p>
					</div>
					<div class="border-b border-white/10 pb-6">
					...
					<div class="border-b border-white/10 pb-6">
						...
						<p class="text-slate-400 text-sm leading-relaxed">Tak. Plan Pro jest miesięczny — po upływie opłaconego okresu konto wraca automatycznie do planu Free.</p>
					</div>
					<div>
						<h3 class="text-white font-semibold mb-2">Co z kluczem API do Google Maps?</h3>
						<p class="text-slate-400 text-sm leading-relaxed">Aplikacja używa naszego klucza API — nie musisz konfigurować nic po swojej stronie. Skany Google Maps są wliczone w Twój limit.</p>
					</div>
				</div>
			</div>
		</div>

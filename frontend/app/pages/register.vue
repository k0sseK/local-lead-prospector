...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = err.response?.data?.detail || "Rejestracja nie powiodła się. Spróbuj ponownie."
  } finally {
    loading.value = false
  }
}
  ...
  <div class="flex-1 flex items-center justify-center py-16 px-4">
    <div class="w-full max-w-md">
      <div class="flex justify-center mb-8">
        <div class="w-12 h-12 rounded-xl bg-brand-green flex items-center justify-center text-black font-extrabold text-2xl shadow-[0_0_20px_rgba(56,239,125,0.3)]">
          L
        </div>
      </div>

      <div class="text-center mb-8">
        <h1 class="text-3xl font-extrabold text-white mb-2">Utwórz konto</h1>
        <p class="text-slate-400 text-sm">Zacznij za darmo — bez karty kredytowej</p>
      </div>

      <div class="bg-brand-card border border-brand-teal/20 rounded-2xl p-8 space-y-5 shadow-[0_20px_60px_rgba(0,0,0,0.4)]">
        <div class="space-y-2">
        ...
        <div class="space-y-2">
          <label for="password" class="text-sm font-medium text-slate-300">Hasło</label>
          <input
            id="password"
            type="password"
            v-model="password"
      ...
      <p class="text-center text-sm text-slate-500 mt-6">
        Masz już konto?
        <NuxtLink to="/login" class="text-brand-green hover:brightness-110 font-medium transition-colors">
          Zaloguj się
        </NuxtLink>
      </p>
    </div>

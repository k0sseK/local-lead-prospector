...
const scanLimitReached = computed(
	() =>
		usage.value &&
		usage.value.total_credits < (usage.value.action_costs?.scan ?? 1),
);

const auditLimitReached = computed(
	() =>
		usage.value &&
		usage.value.total_credits < (usage.value.action_costs?.ai_audit ?? 3),
);
// ─────────────────────────────────────────────────────────────────
					...
					>
						...
						<div class="mt-auto">
							<div class="mt-4">
								...
								<button
									v-else
									@click.stop="auditSingle(lead)"
									:disabled="auditingIds.has(lead.id)"
									class="w-full text-xs font-semibold py-1.5 bg-brand-green/10 text-brand-teal rounded hover:bg-brand-green/20 border border-brand-green/20 flex items-center justify-center gap-1 transition-colors disabled:opacity-60"
								>
